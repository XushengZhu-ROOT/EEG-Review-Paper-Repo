...
# 任务名称（会用于输出文件夹命名）
TASK_NAME = "EEGPT_Stress"

# 数据路径（Stress 数据目录位置，建议结构为 Stress_data/{train,val,test}）
data_root = "Stress_data"

# 分类类别数（Stress: 0, 1 二分类）
num_classes = 2
...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(
        ...
        self.debug = True
        self.chans_num = len(use_channels_names)  # 30
        ...
        target_encoder = EEGTransformer(
            img_size=[self.chans_num, 5 * 256],  # 30 通道, 5s@256Hz = 1280
            patch_size=32*2,  # 64
            embed_num=4,
            embed_dim=512,
            depth=8,
        ...
        # 输入为 30 通道（Stress数据使用全部30个通道）
        self.chan_conv       = Conv1dWithConstraint(30, self.chans_num, 1, max_norm=1)
        
        # 分类头结构（按之前的实现：直接展平，不先池化）
        # z 的 shape: (B, N, embed_num, embed_dim) = (B, N, 4, 512)
        # 计算patch数量N
        # patch_size=64, patch_stride=None (默认等于patch_size), seq_len=1280
        # 如果patch_stride=None，N = seq_len // patch_size = 1280 // 64 = 20
        ...
        if hasattr(target_encoder, 'num_patches') and isinstance(target_encoder.num_patches, tuple):
            ...
            # 默认计算：假设patch_stride=None，seq_len=1280, patch_size=64
            seq_len = 1280
            patch_size = 64
            self.N = seq_len // patch_size  # 1280 // 64 = 20
            if self.debug:
...
def main():
    ...
    class_counts, n_tot = class_stats(train_dir, num_classes=num_classes, expected_channels=30)
    # 计算类别权重：逆频率加权
    class_weights = []
