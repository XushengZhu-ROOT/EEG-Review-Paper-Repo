...
class KaggleEEGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir: str, split: str = "train", expected_channels: int = 62, target_channels: int = 60):
        ...
        if skipped_channels > 0:
            print(f"[KaggleEEGDataset] split={split}, 跳过 {skipped_channels} 个通道数不匹配的文件")
        if skipped_neutral > 0:
            print(f"[KaggleEEGDataset] split={split}, 跳过 {skipped_neutral} 个neutral(2)样本")
        print(f"[KaggleEEGDataset] split={split}, 样本数={len(self.paths)}")
...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(
        ...
        target_encoder = EEGTransformer(
            ...
            patch_size=32*2,  # 64
            patch_stride=None,  # 不使用patch_stride，按motor和kaggle的方式
            embed_num=4,
            embed_dim=512,
            depth=8,
            num_heads=8,
...
def main():
    ...
    train_dataset = KaggleEEGDataset(data_root, split="train", expected_channels=62, target_channels=60)
    valid_dataset = KaggleEEGDataset(data_root, split="val", expected_channels=62, target_channels=60)
    ...
    if test_dir.is_dir():
        ...
        test_dataset = KaggleEEGDataset(data_root, split="test", expected_channels=62, target_channels=60)
