...
class BinaryClassifierHead_CbramodStyle_3Ly_BCE(nn.Module):
    def __init__(self, embed_dim=200, channel_size=30, window_size=5, dropout=0.5, init_scale=0.001):
        ...
        self.fc_layer3 = nn.Linear(embed_dim, 6)  # Layer 3: embed_dim -> 6
...
class NeuralTransformer_Cbramod3lyClassifier_BCE(nn.Module):
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        # self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.head = BinaryClassifierHead_CbramodStyle_3Ly_BCE(embed_dim=self.embed_dim, channel_size=self.channel_size, window_size=self.window_size, dropout=self.classifier_dropout) if num_classes > 0 else nn.Identity() # 150 = 30 channel * 5 seconds
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        # print("patch_embed output:", x.shape) #[499, 20, 200])
        # print("input_time_window:", input_time_window) #1
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
