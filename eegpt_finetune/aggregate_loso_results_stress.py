...
(review-finetune-stress-LOSO.sh), reading directly from the
{task}_{model}_fold{i:02d}.json / .npz pairs saved by
save_eegpt_stress_fold_results() in linear_probe_EEGPT_Stress.py.

Ported from cbramod_finetune/aggregate_loso_results_stress.py (same npz/json
schema, same self-consistency check against the sidecar json's
balanced_accuracy, same POOLED section) with defaults switched to the eegpt
fold_results directory/model name. Two Stress-specific additions on top of
the plain Motor/Motion aggregate_loso_results.py:

1. roc_auc/pr_auc per fold, computed from the stored y_prob (N,2) array
   (eegpt's Stress head is already a genuine 2-class softmax output, same as
   Motor, so y_prob[:, 1] is a real positive-class probability -- unlike
   cbramod/labram/neurolm's single-logit+sigmoid heads, no reconstruction
   needed).
2. A POOLED section: 11 of the 17 stress subjects only ever recorded one
   condition (increase-only or normal-only -- see subject_edf_mapping.csv),
   so whichever subject is held out as test/val in a given fold is often
   single-class. cohen_kappa_score/roc_auc_score are undefined on
   single-class data, so a per-fold mean +/- SD is NaN-polluted or
   misleading for Kappa/ROC-AUC/PR-AUC. The POOLED section concatenates all
   17 folds' predictions (every subject contributes to test exactly once,
   the defining property of LOSO) and computes ONE overall
   accuracy/balanced_accuracy/kappa/macro_f1/per_class_recall/confusion_matrix/
   roc_auc/pr_auc from that -- this is the number to report, not the
   per-fold mean. As a sanity check, the per-fold-summed confusion matrix
   must exactly equal the pooled confusion matrix.

Usage:
  python3 aggregate_loso_results_stress.py
  python3 aggregate_loso_results_stress.py --fold_results_dir ./fold_results_eegpt_stress --task stress --model eegpt --out loso_results_eegpt_stress.csv
"""
import argparse
import csv
import glob
...
def main():
    ...
    parser.add_argument("--fold_results_dir", type=str, default="./fold_results_eegpt_stress")
    parser.add_argument("--task", type=str, default="stress")
    parser.add_argument("--model", type=str, default="eegpt")
    parser.add_argument("--out", type=str, default="loso_results_eegpt_stress.csv")
    parser.add_argument("--cm_out", type=str, default="loso_confusion_matrix_eegpt_stress.npy")
