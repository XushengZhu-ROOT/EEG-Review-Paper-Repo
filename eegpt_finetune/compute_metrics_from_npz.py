...
def compute_metrics(npz_path, n_classes):
    ...
    # AUROC/AUPRC 只对二分类任务（n_classes==2，如 Stress/CustomStress/KaggleERN）算：
    # y_prob 存的是 (N, 2) 的 [P(class0), P(class1)]，用正类概率 y_prob[:,1] 算。
    # 某一折的 y_true 只有单一类别时（Stress 17 个受试者里有 11 个只做过 increase 或
    # 只做过 normal，很常见），roc_auc_score 无定义，返回 NaN 而不是抛异常——跟
    # cbramod_finetune/finetune_evaluator.py 里训练时的处理方式一致。
    if n_classes == 2:
