...
# ===================== finetune_eegpt_sleep.py =====================
# Fine-tunes an EEGPT-like encoder on Sleep 5-class windows stored as pickles:
#   {"signal": np.ndarray(C, T), "label": 0-4}
...
# Sleep task uses 6 channels: C3, C4, F3, F4, O1, O2
...
class Config:
    ...
    # Saving
    output_dir: str = "./sleep_output/eegpt_linear_probe"
...
class PickleWindowDataset(Dataset):
    """
    Expects each pickle to be a dict: {"signal": np.ndarray(C, T), "label": 0-4}.
    Loads raw (C,T), applies NaN/Inf sanitization only; all preprocessing is in-model.
    - Only accepts 6-channel data (C3, C4, F3, F4, O1, O2)
    - Data length: 30 seconds (7680 samples at 256Hz)
    """
    def __init__(self, folder: str, expected_channels: int = 6, target_channels: int = 6):
    ...
    def __getitem__(self, idx):
        ...
        # Sleep 5-class format
        X = np.asarray(obj["signal"], dtype=np.float32)  # (C, T)
        y = int(obj["label"])  # 0-4 for 5 classes
        ...
        # Use all channels (6 channels for sleep task)
...
def make_loaders(cfg: Config, num_classes: int = 5):
    ...
    ds_train = PickleWindowDataset(train_dir, expected_channels=6, target_channels=6)
    ds_val   = PickleWindowDataset(val_dir, expected_channels=6, target_channels=6)
    ds_test  = PickleWindowDataset(test_dir, expected_channels=6, target_channels=6)
...
class EEGPTStressClassifier(nn.Module):
    """
    Match EEGPT official finetuning for Sleep 5-class task:
      - Preproc: x/10, subtract mean across channels, interpolate T->512
      - Channel 1x1 conv: in_channels -> 6 (constrained conv in paper; plain Conv1d here)
      - Encoder: (C=6, T=512), patch=64, stride=32, embed_num=4, embed_dim=512, depth=8, heads=8
      - MLP Head: Layer1(N×embed_num×embed_dim->N×embed_dim), Layer2(N×embed_dim->embed_dim), Layer3(embed_dim->5)
    """
    def __init__(self, cfg: Config, detected_in_channels: int):
        ...
        # 1) channel conv to the 6 sleep channels (use a simple 1x1 conv)
        ...
        # 2) EEGPT encoder (use 6x512 view, but keep summary tokens & widths from ckpt)
        self.encoder = EEGTransformerEncoder(
            img_size=(len(use_channels_names), cfg.eegpt_seq_len),   # (6, 512)
        ...
        #    Layer 3: embed_dim -> num_classes (6)
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(self.cfg.embed_dim, 5)  # 5 classes for sleep task
        )
    ...
    def forward(self, x):
        ...
        # map to 6 canonical channels via constrained 1x1 conv
        x = self.chan_conv(x)
...
def main():
    ...
    for idx, (bs, lr, wd) in enumerate(product(BS_LIST, LR_LIST, WD_LIST), start=1):
        ...
        train_loader, val_loader, test_loader, class_weights = make_loaders(cfg, num_classes=5)
        ...
        for epoch in range(cfg.epochs):
            ...
            val_metrics = evaluate(model, val_loader, device, num_classes=5)
            per_class_str = ", ".join([f"c{i}:{val_metrics['per_class_acc'][i]:.3f}" for i in range(5)])
        ...
        test_metrics = evaluate(model, test_loader, device, num_classes=5)
        print("========== TEST ==========")
