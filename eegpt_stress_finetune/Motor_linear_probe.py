...
# ===================== finetune_eegpt_motor.py =====================
# Fine-tunes an EEGPT-like encoder on Motor 6-class windows stored as pickles:
#   {"signal": np.ndarray(C, T), "label": 0-5}
...
use_channels_names = [
    ...
    'P7','PZ','P4','T7','P3','O1','O2','C4','T8'
]
# Note: A2 is not in CHANNEL_LIST, so we discard it (last channel in 20-channel data)
...
class Config:
    ...
    # Saving
    output_dir: str = "./motor_output/eegpt_linear_probe"
...
class PickleWindowDataset(Dataset):
    """
    Expects each pickle to be a dict: {"signal": np.ndarray(C, T), "label": 0-5}.
    Loads raw (C,T), applies NaN/Inf sanitization only; all preprocessing is in-model.
    - Only accepts 20-channel data (discards others)
    - Discards last channel (A2) to get 19 channels
    """
    def __init__(self, folder: str, expected_channels: int = 20, target_channels: int = 19):
    ...
    def __getitem__(self, idx):
        ...
        # Motor 6-class format
        X = np.asarray(obj["signal"], dtype=np.float32)  # (C, T)
        y = int(obj["label"])  # 0-5 for 6 classes
        ...
        # Discard last channel (A2) to get target_channels
        X = X[:self.target_channels, :]  # (target_channels, T)

        # Sanitize: replace NaN/Inf and clamp extreme outliers defensively
        X = np.nan_to_num(X, posinf=0.0, neginf=0.0)

        X = torch.from_numpy(X)                  # (target_channels, T)
...
def class_stats(folder: str, num_classes: int = 6):
    """Compute class distribution for class weights."""
...
def make_loaders(cfg: Config, num_classes: int = 6):
    ...
    class_counts, n_tot = class_stats(train_dir, num_classes=num_classes)
...
class EEGPTStressClassifier(nn.Module):
    """
    Match EEGPT official finetuning for Motor 6-class task:
      ...
      - Head: Dropout(0.5) -> Linear(2048->16) per patch, flatten (N*16), Linear(240->6)
    """
    def __init__(self, cfg: Config, detected_in_channels: int):
        ...
        # 1) channel conv to the 19 motor channels (use a simple 1x1 conv)
        ...
        # 2) EEGPT encoder (use 19x512 view, but keep summary tokens & widths from ckpt)
        self.encoder = EEGTransformerEncoder(
            img_size=(len(use_channels_names), cfg.eegpt_seq_len),   # (19, 512)
        ...
        # 3) Official probe head: 2048 -> 16 per patch, then (N*16) -> 6 (6-class motor task)
        ...
        self.linear_probe2 = LinearWithConstraint(self.N * 16, 6, bias=True, max_norm=2.0)  # 6 classes for motor task
    ...
    def forward(self, x):
        ...
        logits = self.linear_probe2(h)                       # (B, 6)
        return logits
...
def main():
    ...
    for idx, (bs, lr, wd) in enumerate(product(BS_LIST, LR_LIST, WD_LIST), start=1):
        ...
        if cfg.probe_only:
            # Official behavior: train only chan_conv + probe head
            trainable_params = list(base.chan_conv.parameters()) \
                            + list(base.linear_probe1.parameters()) \
