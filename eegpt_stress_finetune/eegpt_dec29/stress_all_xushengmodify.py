...
GPU_VISIBLE = "3,4,5"        # e.g., "0,1" or "4"
...
class EEGPTStressClassifier(nn.Module):
    ...
    Match EEGPT official finetuning:
      ...
      - Simple Head: Rearrange -> LayerNorm -> Linear (output: 2 for binary classification)
    """
    def __init__(self, cfg: Config, detected_in_channels: int):
        ...
        # 3) Simple classifier head: LayerNorm + Linear
        ...
        self.classifier = nn.Sequential(
            ...
            # LayerNorm
            nn.LayerNorm(self.N * self.cfg.embed_num * self.cfg.embed_dim),
            # Linear: (N * embed_num * embed_dim) -> 2 (for binary classification with CrossEntropyLoss)
            nn.Linear(self.N * self.cfg.embed_num * self.cfg.embed_dim, 2)
        )
...
def main():
    ...
    for idx, (bs, lr, wd) in enumerate(product(BS_LIST, LR_LIST, WD_LIST), start=1):
        ...
        if cfg.probe_only:
            # 只訓練 classifier head（因為你已經移除 chan_conv）
            trainable_params = list(base.classifier.parameters())
