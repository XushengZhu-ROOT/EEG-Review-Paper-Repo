...
review-finetune-Motion-LOSO.sh (see save_motion_fold_results() in
run_multiclass_supervised.py).

Unlike cbramod/aggregate_loso_results_R2.py (which reads config.yaml +
training_summary.json under each fold's model_dir), this reads directly
from the {task}_{model}_fold{i:02d}.json / .npz pair written by
save_motion_fold_results() -- everything needed (test_subject, val_subject,
...
Usage:
  ...
  python3 aggregate_loso_results.py --fold_results_dir ./fold_results_biot --task motion --model biot --out loso_results_biot.csv
  python3 aggregate_loso_results.py --fold_results_dir ./fold_results_st --task motion --model st --out loso_results_st.csv
"""
import argparse
import glob
import json
