...
# -- NO_preprocess + keep-zscore variant: isolates the offline preprocessing
#    (bandpass/DC-removal/detrend/average-reference, done in
#    motiondata_preprocess_copy.ipynb) from the in-code amplitude
#    normalization. Reuses the exact same AllSubjects_Epochs_NO_preprocess
#    data as review-finetune-motor6class-LOSO_no_Preprocess.sh (the dataset
#    on disk is unaffected by --do-normalization, which only acts on
#    already-loaded data at train time -- see Motor6ClassDataset.get_trials_all()
#    / EEGDataset.preprocess_sample() in src/batcher/), but leaves
#    --do-normalization at its default True, so the in-code z-score still
#    runs. This lets you tell apart "offline preprocessing matters" from
#    "turning off normalization broke training" (the first no-preprocess run
#    collapsed to near-chance balanced accuracy on 18/20 folds, most likely
#    because un-normalized raw-amplitude EEG is far outside what the
#    pretrained NeuroGPT encoder expects).
# =============================================================================
set -e
...
exp_name="LOSO_noPreprocess_keepZscore_full_3ly_lr${lr}_wd${wd}_bs${batch_size}"
fold_results_dir="./fold_results_neurogpt_noPreprocess_keepZscore"
...
for (( i=0; i<N; i++ )); do
    ...
    --log-dir="./results/${dataset}_LOSO_noPreprocess_keepZscore" \
