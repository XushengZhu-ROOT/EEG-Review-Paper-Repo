...
class EEGConformer(EEGModuleMixin, nn.Module):
    ...
    def __init__(
        ...
        if self.is_decoding_mode:
            ...
            print(f"self.n_outputs is {self.n_outputs}")
            # self.fc = _FullyConnected(final_fc_length=final_fc_length)
            if cls_head_layer == "3ly":
                self.fc = _FullyConnected_3ly(final_fc_length=final_fc_length)
                self.final_layer = _FinalLayer(
                    n_classes=self.n_outputs,
                    return_features=return_features,
                    add_log_softmax=self.add_log_softmax,
                )
            elif cls_head_layer == "1ly":
                self.fc = _FullyConnected_1ly(final_fc_length=final_fc_length)
                self.final_layer = _FinalLayer(
                    n_classes=self.n_outputs,
                    hidden_channels=final_fc_length * 2,
                    return_features=return_features,
                    add_log_softmax=self.add_log_softmax,
                )
            else:
                raise ValueError(f"undefined cls_head_layer: {cls_head_layer}")
...
class _PatchEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.projection = nn.Sequential(
            ...
            Rearrange(
                "b d_model 1 seq -> b seq d_model"
            ),  # no need, because it will be flattened
        )
...
class _FullyConnected_3ly(nn.Module):
    def __init__(
        self,
        final_fc_length,
        drop_prob_1=0.1,
...
class _FullyConnected_1ly(nn.Module):
    def __init__(
        self,
        final_fc_length,
        drop_prob_1=0.1,
