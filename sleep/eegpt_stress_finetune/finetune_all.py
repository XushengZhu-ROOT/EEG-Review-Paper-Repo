...
GPU_VISIBLE = "1,2,3"        # e.g., "0,1" or "4"
...
class Conv1dWithConstraint(nn.Conv1d, _MaxNormMixin):
    def __init__(self, *args, max_norm: float = 2.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_norm = max_norm
    def forward(self, x):
        # weight: (out_channels, in_channels, kernel_size)
        # self._enforce_max_norm(self.weight, self.max_norm, dim=(1, 2))
        return super().forward(x)

class LinearWithConstraint(nn.Linear, _MaxNormMixin):
    def __init__(self, *args, max_norm: float = 2.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_norm = max_norm
    def forward(self, x):
        # self._enforce_max_norm(self.weight, self.max_norm, dim=1)
        return super().forward(x)
...
class Config:
    ...
    data_root: str = "/work/HHRI-AI/UCSD_EEG/eeg_data/EEG_data/EEGPT_Data/KaggleERN/s42_n56-eegpt"
    train_dir: str = "train"
    val_dir:   str = "val"
    test_dir:  str = "test"
    ...
    # Saving
    output_dir: str = "./3layer_results/eegpt_all"
...
def temporal_interpolation(x: torch.Tensor, target_len: int) -> torch.Tensor:
    """
    x: (B, C, T). Matches the official util's behavior closely:
    - linear interpolation over time
    - align_corners=False
    - handles no-op if already target_len
    """
...
class EEGPTStressClassifier(nn.Module):
    ...
    def __init__(self, cfg: Config, detected_in_channels: int):
        ...
        self.register_buffer("chan_ids", torch.arange(len(use_channels_names)).view(1, -1).long())
...
class EEGPTSleepClassifier(nn.Module):
    """
    Match EEGPT official finetuning:
      - Preproc: x/10, subtract mean across channels, interpolate T->512
      - Channel 1x1 conv: in_channels -> 19 (constrained conv in paper; plain Conv1d here)
    ...
    def __init__(self, cfg: Config, detected_in_channels: int):
        ...
        self.drop = nn.Dropout(p=0.50)
        self.linear_probe1 = LinearWithConstraint(self.cfg.embed_num * self.cfg.embed_dim, 16, bias=True, max_norm=2.0)
        self.linear_probe2 = LinearWithConstraint(self.N * 16, 5, bias=True, max_norm=2.0)
    ...
    def forward(self, x):
        ...
        h = self.linear_probe1(self.drop(h))                 # (B, N, 16)
        h = h.flatten(1)                                     # (B, N*16) == (B, 240)
        logits = self.linear_probe2(h)                       # (B, 5)
        return logits
...
def main():
    ...
    for idx, (bs, lr, wd) in enumerate(product(BS_LIST, LR_LIST, WD_LIST), start=1):
        ...
        # ----- Build+wrap model -----
        model = EEGPTSleepClassifier(cfg, detected_in_channels=detected_C).to(device)
        ...
        base = _unwrap(model)
        with torch.no_grad():
            if isinstance(base.chan_conv, Conv1dWithConstraint):
                base.chan_conv._enforce_max_norm(base.chan_conv.weight, base.chan_conv.max_norm, dim=(1, 2))
            base.linear_probe1._enforce_max_norm(base.linear_probe1.weight, base.linear_probe1.max_norm, dim=1)
            base.linear_probe2._enforce_max_norm(base.linear_probe2.weight, base.linear_probe2.max_norm, dim=1)
