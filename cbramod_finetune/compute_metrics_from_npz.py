...
用法：
  ...
  python3 compute_metrics_from_npz.py --npz_dir fold_results --task motortask --model cbramod
...
def compute_metrics(npz_path, n_classes, ci=False, n_bootstrap=1000, ci_seed=0):
    ...
    # AUROC/AUPRC/F1(正类) 只对二分类任务（n_classes==2，如 CustomStress/KaggleERN）算：
    ...
    # f1_pos（正类 binary F1，跟上面两类平均的 macro_f1 不是一回事）在类别不均衡的
