...
(scripts/review-finetune-stress-LOSO.sh), reading directly from the
{task}_{model}_fold{i:02d}.json / .npz pairs saved by
finetune_trainer.py's save_fold_results() / finetune_evaluator.py's
save_fold_predictions_npz().

Adapted from aggregate_loso_results.py (same npz/json schema, same
self-consistency check against the sidecar json's balanced_accuracy) with
two Stress-specific additions:

1. roc_auc/pr_auc per fold, computed from the stored y_prob (N,2) array.
2. A POOLED section: unlike Motor, 11 of the 17 stress subjects only ever
   recorded one condition (increase-only or normal-only -- see
   subject_edf_mapping.csv), so whichever subject is held out as test/val in
   a given fold is often single-class. cohen_kappa_score/roc_auc_score are
   undefined on single-class data (return/require NaN), so a per-fold
   mean +/- SD is NaN-polluted or misleading for Kappa/ROC-AUC/PR-AUC (verified
   directly: 11/17 folds are single-class). The POOLED section concatenates
   all 17 folds' predictions (every subject contributes to test exactly once,
   ...
   roc_auc/pr_auc from that -- this is the number to report, not the per-fold
   mean. As a sanity check, the per-fold-summed confusion matrix must exactly
   equal the pooled confusion matrix (both partition the same total sample set).

Usage:
  python3 aggregate_loso_results_stress.py
  python3 aggregate_loso_results_stress.py --fold_results_dir ./fold_results_stress --task stress --model cbramod --out loso_results_cbramod_stress.csv
"""
import argparse
import csv
import glob
...
def main():
    ...
    parser.add_argument("--fold_results_dir", type=str, default="./fold_results_stress")
    parser.add_argument("--task", type=str, default="stress")
    parser.add_argument("--model", type=str, default="cbramod")
    parser.add_argument("--out", type=str, default="loso_results_cbramod_stress.csv")
    parser.add_argument("--cm_out", type=str, default="loso_confusion_matrix_cbramod_stress.npy")
