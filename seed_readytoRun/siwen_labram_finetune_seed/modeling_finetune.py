...
class BinaryClassifierHead_CbramodStyle_3Ly_BCE(nn.Module):
    def __init__(self, embed_dim=200, channel_size=30, window_size=5, dropout=0.5, init_scale=0.001, num_classes=6):
        super().__init__()
        ...
        # 延迟初始化：第一层 Linear 将在第一次 forward 时根据实际输入维度创建
        self.fc_layer1 = None
        self.activation = nn.ELU()
        self.dropout_layer = nn.Dropout(dropout)
        
        # 预先定义后续层（这些层的维度是固定的）
        self.fc_layer2 = nn.Linear(window_size * embed_dim, embed_dim)  # Layer 2: window_size*embed_dim -> embed_dim
        self.fc_layer3 = nn.Linear(embed_dim, num_classes)  # Layer 3: embed_dim -> num_classes
        
        if init_scale != 1.0:
            if isinstance(self.fc_layer3, nn.Linear):
                self.fc_layer3.weight.data.mul_(init_scale)
                if self.fc_layer3.bias is not None:
                    self.fc_layer3.bias.data.mul_(init_scale)

    def _build_first_layer(self, input_dim):
        """根据实际输入维度创建第一层 Linear"""
        if self.fc_layer1 is None or self.fc_layer1.in_features != input_dim:
            self.fc_layer1 = nn.Linear(input_dim, self.window_size * self.embed_dim)
            # 将层移动到正确的设备
            if self.fc_layer2 is not None:
                device = next(self.fc_layer2.parameters()).device
                self.fc_layer1 = self.fc_layer1.to(device)
            # 初始化权重
            nn.init.xavier_uniform_(self.fc_layer1.weight)
            if self.fc_layer1.bias is not None:
    ...
    def forward(self, x):
        ...
        # Flatten: [batch_size, num_patches * embed_dim]
        x_flat = x.view(batch_size, -1)
        actual_input_dim = x_flat.shape[1]
        
        # 根据实际输入维度动态创建第一层
        self._build_first_layer(actual_input_dim)
        
        # Layer 1: 动态输入维度 -> window_size * embed_dim
        x = self.fc_layer1(x_flat)
        x = self.activation(x)
        x = self.dropout_layer(x)
        
        # Layer 2: window_size * embed_dim -> embed_dim
        x = self.fc_layer2(x)
        x = self.activation(x)
        x = self.dropout_layer(x)
...
class NeuralTransformer_Cbramod3lyClassifier_BCE(nn.Module):
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        # self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.head = BinaryClassifierHead_CbramodStyle_3Ly_BCE(embed_dim=self.embed_dim, channel_size=self.channel_size, window_size=self.window_size, dropout=self.classifier_dropout, num_classes=num_classes) if num_classes > 0 else nn.Identity() # 150 = 30 channel * 5 seconds
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        pos_embed_used = self.pos_embed[:, input_chans] if input_chans is not None else self.pos_embed
        # print("pos_embed_used shape =", pos_embed_used.shape) #[1, 21, 200] [1, 129, 200]

        if self.pos_embed is not None:
            pos_embed = pos_embed_used[:, 1:, :].unsqueeze(2).expand(batch_size, -1, input_time_window, -1).flatten(1, 2)
            pos_embed = torch.cat((pos_embed_used[:,0:1,:].expand(batch_size, -1, -1), pos_embed), dim=1)
            x = x + pos_embed
            # print("x+pos_embed shape =", x.shape) #[499, 21, 200]
            # print("time_embed",self.time_embed)
        if self.time_embed is not None:
            nc = n if t == self.patch_size else a
            time_embed = self.time_embed[:, 0:input_time_window, :].unsqueeze(1).expand(batch_size, nc, -1, -1).flatten(1, 2)
            x[:, 1:, :] += time_embed
            # print("x+time_embed shape =", x.shape)

        x = self.pos_drop(x)
        # print("X before blk:",x.shape) #[499, 21, 200]
        for blk in self.blocks:
            x = blk(x, rel_pos_bias=None)
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        '''
        # x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
        x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=True, return_all_tokens=False, **kwargs)
        x = self.head(x)
        # print("1")
        return x
...
def labram_base_patch200_200_cbramod3lyclassifier(pretrained=False, **kwargs):
    model = NeuralTransformer_Cbramod3lyClassifier_BCE(
        EEG_size=4000,patch_size=200, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
