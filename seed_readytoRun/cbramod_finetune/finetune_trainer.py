...
class Trainer(object):
    ...
    def train_for_multiclass(self):
        ...
        print("="*70)
        print(f"Best model from Epoch {best_metrics['epoch']} (selected by Val BACC)")
        ...
        self.save_best_results_file(best_metrics)
    ...
    def save_best_results_file(self, best_metrics):
        """保存最佳結果摘要文件（文件名包含關鍵信息，方便快速查看）"""
        if not os.path.isdir(self.params.model_dir):
            os.makedirs(self.params.model_dir)
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Selection Criteria: Val BACC\n\n")
